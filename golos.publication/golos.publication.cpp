...
void publication::set_vote(name voter, name author, string permlink, int16_t weight) {
    ...
    auto votetable_index = vote_table.template get_index<"combine"_n>();
    auto vote_itr = votetable_index.find(structures::voteinfo::combine_fields(hash64(permlink), voter));
