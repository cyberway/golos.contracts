...
void publication::create_message(structures::mssgid message_id,
                              structures::mssgid parent_message_id,
    ...
    if (parent_message_id.author) {
        if (social_acc_param.account) {
            eosio_assert(!social::is_blocking(social_acc_param.account, parent_message_id.author, message_id.author), "You are blocked by this account");
        }
    }
    ...
    use_charge(lims, parent_message_id.author ? structures::limitparams::COMM : structures::limitparams::POST, issuer, message_id.author,
    ...
    if (id_message == 0)
        id_message = 1;
    if(!parent_message_id.author)
        use_postbw_charge(lims, issuer, message_id.author, token_code, id_message);
    ...
    if (parent_message_id.author) {
        tables::message_table message_table(_self, parent_message_id.author.value);
        auto message_index = message_table.get_index<"bypermlink"_n>();
        auto message_itr = message_index.find({parent_message_id.permlink, parent_message_id.ref_block_num});
        parent_id = message_itr->id;
    }

    uint8_t level = 0;
    if(parent_message_id.author)
        level = 1 + notify_parent(true, parent_message_id.author, parent_id);
    ...
    auto mssg_itr = message_table.emplace(message_id.author, [&]( auto &item ) {
        ...
        item.date = cur_time;
        item.parentacc = parent_message_id.author;
    ...
    structures::accandvalue parent {parent_message_id.author, parent_id};
    ...
    if (!closed && delay_sec)
        close_message_timer(message_id.author, id_message, delay_sec);
...
void publication::update_message(structures::mssgid message_id,
    ...
    if (is_close_message_by_ref_block_num(message_id.ref_block_num) || message_itr->closed)
        return;
