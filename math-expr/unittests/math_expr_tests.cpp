...
public:
	...
	expr_t get_result() {
		...
		return expr_t::from_data(res["value"].as<base_t>());
	}
   
	action_result calculate(const std::vector<expr_t>& vars) {
		std::vector<base_t> bases(vars.size());
		for(size_t i = 0; i < vars.size(); i++)
			bases[i] = vars[i].data();
		return push_action(N(math.expr), N(calculate), mvo()("vars", reinterpret_cast<const std::vector<base_t>&>(vars)));
	}
	
	action_result set_and_calc(const std::string& expr_str, const std::vector<expr_t>& vars) { 
		produce_blocks(2);
...
BOOST_FIXTURE_TEST_CASE(math_tests, math_expr_tester) try {    
	...
	CHECK_MATH(sqrt(x[0]) + sqrt(x[2]) * sqrt(x[1]) / sqrt(x[2]),        (30.4, 3.15, 2.1));
	CHECK_MATH(((x[0] / x[1]) / (x[2])) * x[3], (30.4, 3.15, 2.1, 1.5));
	CHECK_MATH(abs(x[0]) / sqrt(abs(x[3])) + sqrt(abs(x[2])) + abs(x[1]), (-30.4, -3.15, -2.1, -1.1));
	
	CHECK_MATH(log2(x[0]) + log2(x[2]) * log2(x[1]) / log2(x[3]),        (30.4, 3.15, 2.1, 1.5));
	CHECK_MATH(log10(x[0]) + log10(x[3]) * log10(x[1]) / log10(x[2]),    (30.4, 3.15, 2.1, 1.5));    
	CHECK_MATH(log10(x[0]) + log10(x[3]) * log10(x[1]) / log10(x[2]),    (310.4, 0.15, 5.1, 88.5));
	
	CHECK_MATH_EQUAL(pow(x[2], 2) + pow(x[3], 2) + pow(x[0], 3) * pow(x[1], 4), "(x2^2) + (x3^2) + (x0^3) * (x1^4)", (2, 3, 4, 2));
	
	BOOST_CHECK_EQUAL("assertion failure with message: atmsp::parser: unknown var", set_and_calc("x0 x1", {expr_t(2.0), expr_t(0.0)}));
	BOOST_CHECK_EQUAL("assertion failure with message: atmsp::machine: division by zero", set_and_calc("x0 / x1", {expr_t(2.0), expr_t(0.0)}));
	BOOST_CHECK_EQUAL("assertion failure with message: atmsp::machine: square root of a negative number", set_and_calc("sqrt(x0 - x1)", {expr_t(2.0), expr_t(10.5)}));
	BOOST_CHECK_EQUAL("assertion failure with message: math_expr_test::calculate, wrong input size: 1 != 2", set_and_calc("sqrt(x0 - x1)", {expr_t(10.5)}));
