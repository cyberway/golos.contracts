...
BOOST_FIXTURE_TEST_CASE(basic_tests, golos_charge_tester) try {
    ...
    for (size_t i = 0; i < 10; i++) {
        ...
        BOOST_CHECK_EQUAL(success(), charge.use(cfg::emission_name, _users[0], 'c', 100, 1000));
        produce_block();
    }
    BOOST_TEST_MESSAGE("--- try to use");
    BOOST_CHECK_EQUAL(err.cutoff, charge.use(cfg::emission_name, _users[0], 'c', 100, 1000));
    produce_block();
    BOOST_TEST_MESSAGE("--- waiting 900");
    _produce_block(seconds(900));
    BOOST_TEST_MESSAGE("--- try to use (cutoff == 1000)");
    BOOST_CHECK_EQUAL(err.cutoff, charge.use(cfg::emission_name, _users[0], 'c', 1000, 1000));
    produce_block();
    BOOST_TEST_MESSAGE("--- waiting 100");
    _produce_block(seconds(100));
    BOOST_TEST_MESSAGE("--- use (cutoff == 1000)");
    BOOST_CHECK_EQUAL(success(), charge.use(cfg::emission_name, _users[0], 'c', 1000, 1000));
    produce_block();
