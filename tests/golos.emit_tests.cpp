...
BOOST_FIXTURE_TEST_CASE(start_emission_test, golos_emit_tester) try {
    ...
    auto token_symbol = "{'symbol':'" + std::to_string(_token.decimals()) + "," + _token.name() + "'}";
    auto params = "[" + emit.infrate_json(1500, 95, 250000) + ",['reward_pools'," + pools + "],['emit_token_symbol'," + token_symbol + "]]";
...
BOOST_FIXTURE_TEST_CASE(set_params, golos_emit_tester) try {
    ...
    auto token_symbol = "{'symbol':'" + std::to_string(_token.decimals()) + "," + _token.name() + "'}";
    auto params = "[['inflation_rate'," + infrate + "], ['reward_pools'," + pools + "],['emit_token_symbol'," + token_symbol + "]]";
