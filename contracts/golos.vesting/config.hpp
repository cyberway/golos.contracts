#pragma once

// TODO test params
#define NUMBER_OF_CONVERSIONS 10
#define OUTPUT_PAYOUT_PERIOD 15

//#define NUMBER_OF_CONVERSIONS 13
//#define OUTPUT_PAYOUT_PERIOD 604800

#define TIMEOUT 3
#define NOT_TOKENS_WEEK_PERIOD 0
#define LAST_TOKENS_WEEK_PERIOD 1

#define FRACTION (10000)
#define FULL_PERSENT (1000000)

#define UPPER_BOUND (10000)
#define LOWER_BOUND (0)

#define TWO_DECIMAL_PLACES (100)
#define RECOVERY_PERIOD (432000)

//------------------------------------------

#define NUMBER_OF_CONVERSIONS (13)
#define TOTAL_TERM_OF_CONCLUSION 15
//#define TOTAL_TERM_OF_CONCLUSION (604800)
#define MIN_AMOUNT_VESTING_CONCLUSION
#define MAX_AMOUNT_VESTING_CONCLUSION

#define MIN_AMOUNT_DELEGATION_VESTING
#define MIN_PERIOD_DELEGATION
#define MAX_PERIOD_DELEGATION
#define TIME_LIMIT_FOR_RETURN_DELEGATE_VESTING 15
//#define TIME_LIMIT_FOR_RETURN_DELEGATE_VESTING (604800)
