...
```
При первом вызове операции-действия `setparams` все три типа параметров передаются смарт-контракту вестинг. При повторных вызовах (для изменения настроек) может быть передан только один из трех параметров.  
...
Для выполнения данного операции-действия требуется подпись аккаунта `owner`.  

### Операция-действие withdraw
Операция-действие `withdraw` используется для конвертации количества вестинга в количество токенов. Операция-действие имеет вид:
```cpp
...
Для выполнения данного операции-действия требуется подпись аккаунта `from`.  
