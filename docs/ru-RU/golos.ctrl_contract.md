...
`update_auth` — параметры, определяющие частоту обновления авторизации для аккаунта `multisig_acc`:
  * `period` — период обновления (в секундах). Повторная смена авторизации для аккаунта не выполняется, если с последнего обновления не прошло указанное количество секунд.
