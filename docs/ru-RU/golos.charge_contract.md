...
Смарт-контракт `golos.charge` выполняет прикладную функцию — позволяет ограничивать активность пользователей в создании постов, комментариев, а также в голосовании за посты. Непосредственного взаимодействия пользователя со смарт-контактом `golos.charge` не происходит, обращение к экшен-операциям происходит через смарт контракт публикации.  
...
Обращение к батарейкам происходит через вызовы экшен-операций. В смарт-контракте `golos.charge` реализованы встраиваемая (англ. inline) функция `get_stored` и экшен-операции `use`, `useandstore`, `removestored` и `setrestorer`.  

Одним из обязательных параметров экшен-операций является `token_code`, идентифицирующий конкретный токен. Аккаунт, выполняющий действие с вызовом любой экшен-операции, должен быть наделен правами в соответствии с требованиями авторизации издателя токена, которым может являться либо аккаунт-издатель токена, либо аккаунт, которому аккаунт-издатель токена предоставил соответствующие права.  
...
## Экшен-операция use
Экшен-операция `use` используется для учета ресурса батарейки при выполнении пользователем каких-либо действий.  
Экшен-операция `use` имеет вид:
```cpp
...
Любой из смарт-контрактов может обратиться к батарейке, но не напрямую, а через вызов экшен-операции. В случае, если ресурс батарейки не удовлетворяет значению `cutoff`, смарт-контракт батарейки выдает соответствующее сообщение с блокировкой выполнения транзакции, в том числе действия, вызвавшего обращение к батарейке.  
...
## Экшен-операция useandstore
Экшен-операция `useandstore` также как и `use` используется для учета ресурса батарейки при выполнении пользователем каких либо действий. В отличие от `use` экшен-операция `useandstore` сохраняет значения используемых ресурсов батареек в таблицах, находящихся внутри смарт-контракта `golos.charge`, и остаются доступными по чтению другим смарт-контрактам.  

Экшен-операция `useandstore` имеет вид:
```cpp
...
## Экшен-операция setrestorer
Экшен-операция `setrestorer` используется для задания алгоритма, по которому происходит восстановление ресурса батарейки.  
Экшен-операция `setrestorer` имеет вид:
```cpp
