...
Cмарт-контракт `golos.publication` поддерживает следующие операции-действия пользователей: `setlimit`, `setrules`, `createmssg`, `updatemssg`, `deletemssg`,  `downvote`, `upvote` и `setmaxpayout`.
...
`max_payout` — максимальная выплата из пула за сообщение (суммарная). Задается автором в валюте пула, к которому принадлежит сообщение. Из пула будет выплачено не более, чем эта сумма, даже если начисленная сумма выплаты будет больше. Необязательный параметр, значение по умолчанию = `asset::max_amount`.
...
Операция-действие `setmaxpayout` используется для изменения автором максимальной выплаты в ранее отправленном им сообщение.
...
```
Параметры:  
`message_id` — идентификатор сообщения, состоящий из параметров `author` и `permlink`;  
`max_payout` — максимальная выплата за сообщение. Аналогично такому же полю при создании сообщения.  


Для запуска операции-действия `setmaxpayout` требуется наличие в транзакции подписи аккаунта-автора сообщения.

Максимальную выплату можно изменять только для открытых сообщений.  
Максимальную выплату можно изменять только для сообщений, которые еще не имеют голосов.  
Максимальную выплату можно только уменьшать по сравнению с предыдущей, а не увеличивать и не оставлять без изменений, вызывая операцию-действие `setmaxpayout` впустую.  

## Событие postreward
Событие `postreward` отправляется контрактом при выплате вознаграждения за пост.
Событие `postreward` имеет следующий вид:
```json
{
    "name": "reward_event",
    "base": "",
...
Параметры:
`message_id` — идентификатор поста, состоящий из параметров `author` и `permlink`;
`author_reward` — сумма вознаграждения автору поста;
`benefactor_reward` — общая сумма вознаграждений бенефициарам поста;
`curator_reward` — общая сумма вознаграждений кураторам поста.

Все суммы даются в токенах пула вознаграждений, к которому относится пост.
