...
Для расчёта стоимости поста `payout` небоходимо получить из Event Engine значение `rewardweight`, которое приходит в виде целого числа и оапределяет точность до сотых долей процента. Также при голосовании за пост в Event Engine будет приходить значение текущего состояния поста `poststate`. Необходимо получить из Event Engine сообщение `poolstate` (pool выбирается по времени создания поста). После получения всех необходимых данных воспользоваться **формулой 1**.
### Формула 1
#### payout = rewardweight::reward_weight * poolstate::state.funds * (poststate::sharesfn / poolstate::state::rsharesfn)
Процент кураторских может задаваться пользователем при создании поста, в Event Engine будет отправлено сообщение `poststate`. Если процент не указан, то берется нижняя граница из указанного интервала в параметрах в таблице `pstngparams`. Для получения суммы кураторских вознаграждений следует воспользоваться **формулой 2**. 
#### Формула 2
##### payout -= poststate::sumcuratorsw * payout
Для расчёта кураторских выплат необходимо получить все сообщения Event Engine `votestate`, в которых содержатся информация о куратарах. Из сообщения Event Engine `poststate` необходимо взять общий вес поста. После получения необходимой информации, следует пройти по списку кураторов и для каждого рассчитать выплаты, используя **формулу 3**.
#### Формула 3
...
Затем идёт расчёт выплат бенефециарам, указанным при создании поста. Необходимо пройти по списку и получить выплаты бенефициарам и  общую сумму выплат `ben_payout_sum`, используя **формулу 4**. После расчёта `ben_payout_sum` необходимо получить сумму выплат автору поста по **формуле 5**.
### Формула 4
...
`netshares` - доля награды поста с учётом знака
`voteshares` - доля награды голоса с учётом знака;
`sumcuratorsw` - процент кураторских вознаграждений.
...
`curatorsw` - процент кураторского вознаграждения;
...
`msgs` - колличество постов в пулле вознаграждений;
`funds` - колличество токенов в пулле вознаграждений;
`rshares` - доля награды поста;
